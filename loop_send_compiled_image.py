# 7/12/2021 Allison Moore
# Send a stream of compiled images to Gazebo through ROS
...
#image_check
#Check to see if appropriate number of images are provided for earlier versions
def image_check():
...
#picture_send
#Overarching program for sending provided image to gazebo
def picture_send(background):
    ...
    #Join exisiting path and image path, convert image, append it to image names
...
def image_process():
    ...
    shift_amount = .1
    #end positions
    x_multiplier = [.7+shift_amount, .5, .8+shift_amount,  .2+shift_amount, .05, .3+shift_amount]
    y_multiplier = [.3, .25, .4, .3, .25, .4]
    i = 0
